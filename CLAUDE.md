...
**The Django scaffold exists and core GTD flows are landing.** Epics 0–5 of `docs/task-breakdown.md` are committed (repo bootstrap, Django scaffold/auth/PWA shell, data model, capture web+API, clarify wizard, lists/projects/contexts/ordering/horizons). Project `gtd`, single app `core`, `manage.py` at repo root. Build order continues per `docs/solution-plan.md` Steps 6–12 / `docs/task-breakdown.md` Epics 6–12 — check that file's checkboxes (`[x]` done, `[~]` partial, `[ ]` not started) before assuming a screen or model exists; it is more current than this file.
